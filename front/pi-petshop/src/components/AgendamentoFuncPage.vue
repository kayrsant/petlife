        ...
        <div class="card-content">
          ...
          <div class="cliente-info">
            <p class="cliente-text">
              <strong>Dono:</strong> {{ agendamento.cliente.nome }}
            </p>
          </div>
  ...
  methods: {
    async fetchAgendamentos() {
      try {
        ...
        const response = await axios.get(`${API_URL}/agendamento`, config);
        const agendamentos = response.data.sort((a, b) => {
          const statusOrder = {
            EM_ANDAMENTO: 1,
            ABERTO: 2,
            FINALIZADO: 3,
        ...
        const clientRequests = agendamentos.map(async (agendamento) => {
          const clienteResponse = await axios.get(
            `${API_URL}/cliente/${agendamento.cliente.id}`,
            config
          );
          agendamento.cliente = clienteResponse.data;
          return agendamento;
        });

        this.agendamentos = await Promise.all(clientRequests);
      } catch (error) {
        console.error("Erro ao buscar agendamentos:", error);
      }
    },
    navigateTo(route) {
      this.$router.push({ path: `/func/agendamentos/${route}` });
    },
