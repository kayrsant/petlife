  ...
  methods: {
    async fetchData() {
      try {
        ...
        // Requisição dos dados dos clientes, funcionários e serviços
        const [clientesResponse, funcionariosResponse, servicosResponse] =
          await Promise.all([
            axios.get(`${API_URL}/cliente/todos`, config),
    ...
    async fetchPets() {
      try {
        ...
        const petsResponse = await axios.get(
          `${API_URL}/pet/cliente/${this.selectedCliente}`,
          config
        );
