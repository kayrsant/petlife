  ...
  <div class="edit-pet">
    ...
    <form @submit.prevent="showModal">
      ...
      <div class="form-group">
        ...
        <input type="number" id="idade" v-model="pet.idade" required />
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        ...
        <input type="number" step="0.1" id="peso" v-model="pet.peso" />
      </div>
      <div class="form-group">
      ...
      <div v-if="pet.alergias !== 'Nenhuma'" class="form-group">
    ...
    <div v-if="showConfirmationModal" class="modal">
      <div class="modal-content">
        ...
        <p>
          <strong>Em Tratamento:</strong> {{ pet.emTratamento ? "Sim" : "Não" }}
        </p>
  ...
  methods: {
    async fetchPet() {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8080/petshop/pet/${this.id}`,
          config
        );
    ...
    async updatePet() {
      try {
        ...
        this.pet.peso = parseFloat(this.pet.peso);

        const petData = {
          ...this.pet,
          alergias:
            this.pet.alergias === "Outro"
              ? this.pet.alergiasOutro
              : this.pet.alergias,
          emTratamento: this.pet.emTratamento ? "Sim" : "Não",
        };
